...
public class GMapMigrationComposer : IComposer
{
    public void Compose(IUmbracoBuilder builder) =>
        builder.AddNotificationAsyncHandler<UmbracoApplicationStartingNotification, RunLegacyGMapMigration>();
}

public class RunLegacyGMapMigration(
    ...
    : INotificationAsyncHandler<UmbracoApplicationStartingNotification>
{
    ...
    {
        // Only run if the database is in a state that can accept migrations
        if (runtimeState.Level < RuntimeLevel.Run) return;

        var migrationPlan = new MigrationPlan("Single-GMaps-Universal-Fix");

        migrationPlan.From(string.Empty).To<LegacySingleGmapMigration>("{legacy-latlng-mapcenter-migration}");
...
internal sealed class LegacySingleGmapMigration : AsyncMigrationBase
{
    private readonly ILogger<LegacySingleGmapMigration> _logger;

    public LegacySingleGmapMigration(IMigrationContext context, ILogger<LegacySingleGmapMigration> logger)
    ...
    protected override async Task MigrateAsync()
    {
        ...
        var validPropertyTypeIds = (await Database.FetchAsync<int>(propertyTypeSql)).ToList();
        if (!validPropertyTypeIds.Any()) return;
        ...
        var propertyEntries = (await Database.FetchAsync<GMapPropertyDataDto>(sql)).ToList();
        if (!propertyEntries.Any()) return;
        ...
        foreach (var update in updateBatch)
        {
            ...
            {
                _logger.LogError(ex, "Error transforming GMap row ID: {id}", dto.Id);
                skipList.Add(update);
            }
        }
        ...
        if (updateBatch.Any())
        {
            _logger.LogInformation("Universal Migration: Updating {count} GMaps records.", updateBatch.Count);
